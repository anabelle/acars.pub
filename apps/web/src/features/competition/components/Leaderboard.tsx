...
import { buildLeaderboardRows, sortLeaderboardRows } from '@/features/competition/leaderboardMetrics';
import type { LeaderboardMetric } from '@/features/competition/leaderboardMetrics';

const metricMeta: Record<LeaderboardMetric, { label: string; description: string; isMoney?: boolean }> = {
    ...
    fleetValue: { label: 'Fleet Value', description: 'Ranked by depreciated fleet value', isMoney: true },
...
export function Leaderboard() {
    ...
    const rows = useMemo(() => {
        const aircraftById = new Map([...globalFleet, ...fleet].map((aircraft) => [aircraft.id, aircraft]));
            ...
            <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                {(Object.keys(metricMeta) as LeaderboardMetric[]).map((key) => {
                    ...
                    return (
                        ...
                        >
                            <p className="text-[10px] uppercase font-semibold tracking-wider">{metricMeta[key].label}</p>
                        ...
                        return (
                            ...
                            >
                                <div className="flex w-1/2 items-center gap-3">
                                    <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-border/50 bg-background/60 font-mono text-sm font-black">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">
                                            {row.icaoCode} · {row.ceoPubkey.slice(0, 12)}…
                                        </div>
                                    </div>
                                </div>
                                <div className="flex w-1/2 items-center justify-end gap-6">
                                    ...
                                    <div className="text-right">
                                        <div className="flex items-center justify-end gap-1 text-[10px] uppercase text-muted-foreground">
                                            {metricMeta[metric].label}
                                            {index === 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                                        </div>
