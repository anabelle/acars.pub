...
describe("buildActionDTag", () => {
  it("keeps action-only d-tags for airline create and tick update", () => {
    ...
    expect(createTag).toBe("airtr:world:v4-snapshot-rollup:action:airline_create");
    expect(tickTag).toBe("airtr:world:v4-snapshot-rollup:action:tick_update");
  });
  ...
  it("adds entity id and tick for route actions", () => {
    ...
    expect(tag).toBe("airtr:world:v4-snapshot-rollup:action:route_open:rt-123:100");
  });
  ...
  it("adds entity id, tick, and seq for route actions when seq is provided", () => {
    ...
    expect(tag).toBe("airtr:world:v4-snapshot-rollup:action:route_open:rt-123:100:s7");
  });
  ...
  it("adds entity id and tick for aircraft actions", () => {
    ...
    expect(tag).toBe("airtr:world:v4-snapshot-rollup:action:aircraft_purchase:ac-777:555");
  });
  ...
  it("adds entity id, tick, and seq for aircraft actions when seq is provided", () => {
    ...
    expect(tag).toBe("airtr:world:v4-snapshot-rollup:action:aircraft_purchase:ac-777:555:s42");
  });
  ...
  it("returns base tag when no identifiers are present", () => {
    ...
    expect(tag).toBe("airtr:world:v4-snapshot-rollup:action:hub_add");
  });
  ...
  it("returns base tag with only seq when no identifiers are present but seq is provided", () => {
    ...
    expect(tag).toBe("airtr:world:v4-snapshot-rollup:action:hub_add:s3");
  });
  ...
  it("ensures different seq values produce different d-tags for same action", () => {
    ...
    expect(tag1).toBe("airtr:world:v4-snapshot-rollup:action:route_update_fares:rt-abc:200:s0");
    expect(tag2).toBe("airtr:world:v4-snapshot-rollup:action:route_update_fares:rt-abc:200:s1");
  });
  ...
  it("omits seq suffix when seq is undefined", () => {
    ...
    expect(withoutSeq).toBe("airtr:world:v4-snapshot-rollup:action:route_assign_aircraft:rt-1:50");
  });
});
